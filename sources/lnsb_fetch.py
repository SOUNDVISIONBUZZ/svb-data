...
# Anything else is ignored. No fallback. No Selenium. No JSON-LD.
...
def _segment_bullets(text: str) -> List[str]:
    # Turn bullets into line starts, then split
    text = re.sub(r"\s\*\s+","\n* ", text)
    text = re.sub(r"\s•\s+","\n* ", text)
    parts = re.split(r"(?:^|\n)\*\s+", text)
    segs: List[str] = []
    for p in parts[1:]:  # skip preamble before first bullet
        p = _clean(p)
        if not p: continue
...
def _extract_from_segment(seg: str) -> Optional[Tuple[str,str,str]]:
    # STRICT: only accept exact "Venue - Title - Time" pattern
