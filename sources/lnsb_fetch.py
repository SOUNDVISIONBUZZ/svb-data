...
def _make_id(day: dt.date, venue: str, artist: str) -> str:
    """
    Deterministic ID: lnsb-YYYYMMDD-venue-artist   (all lower-case, 
dashes only)
    """
    stem = f"{day:%Y%m%d}-{venue[:12]}-{artist[:12]}"
    return "lnsb-" + re.sub(r"[^a-z0-9]+", "-", stem.lower()).strip("-")

HDR_RE = re.compile(r"^[A-Z][a-z]+day – \w+ \d", re.I)   # e.g. “TUESDAY 
– August 5”
...
def lnsb_fetch(city_filter: str | None = None) -> list[dict]:
    ...
    for h4 in soup.find_all("h4"):
        ...
        _, _, when = hdr.partition("–")                   # “TUESDAY – 
August 5”
        day = dt.datetime.strptime(when.strip() + " 2025", "%B %d 
%Y").date()
        ...
        while ptr and ptr.name == "p":
            ...
            # Expected pattern:  *Venue – Artist – 5-8 pm   (dashes or 
asterisks vary)
            m = re.match(
...
r".*?[*–]\s*(.+?)\s*–\s*(.+?)\s*–\s*(\d+)(?::(\d+))?\s*([ap]m)",
            ...
            if m:
                ...
                start_dt = dt.datetime.combine(day, dt.time(hr, 
int(minute)))
                ...
                if city_filter and city_filter.lower() not in 
city.lower():
...
def _make_id(day: dt.date, venue: str, artist: str) -> str:
    ...
    return "lnsb-" + re.sub(r"[^a-z0-9]+", "-", 
stem.lower()).strip("-")

HDR_RE = re.compile(r"^[A-Z][a-z]+day – \w+ \d", re.I)   # e.g. 
“TUESDAY – August 5”
...
def lnsb_fetch(city_filter: str | None = None) -> list[dict]:
    print("• LiveNotesSB fetch")
    html = requests.get(SITE, headers={"User-Agent": 
"Mozilla/5.0"}, timeout=20).text
    soup = BeautifulSoup(html, "html.parser")

    # locate each date-section (<h4> heading followed by 
paragraphs)
    events: list[dict] = []
    for h4 in soup.find_all("h4"):
        hdr = h4.get_text(" ", strip=True).replace("\xa0", " ")
        if not HDR_RE.match(hdr):                       # skip 
unrelated h4s
            continue

        # ── parse the yyyy-mm-dd out of the heading
        _, _, when = hdr.partition("–")                 # “TUESDAY 
– August 5”
        day = dt.datetime.strptime(when.strip() + " 2025", "%B %d 
%Y").date()

        # walk the siblings (<p> tags) until next heading
        ptr = h4.find_next_sibling()
        while ptr and ptr.name == "p":
            txt = ptr.get_text(" ", strip=True)

            # format is “*Venue – Artist (style) – 5-8 pm”
            m = 
re.match(r".*?[*–]\s*(.+?)\s*–\s*(.+?)\s*–\s*(\d+)(?::(\d+))?\s*([ap]m)", 
txt, re.I)
            if m:
                venue, artist, hr, minute, ampm = m.groups()
                minute = minute or "00"
                hr = int(hr) % 12 + (12 if ampm.lower() == "pm" 
else 0)
                start_dt = dt.datetime.combine(day, dt.time(hr, 
int(minute)))
                end_dt   = start_dt + dt.timedelta(hours=2)

                city = "Santa Barbara"                  # default
                if "–" in venue:
                    city = venue.split("–")[0].strip()
...
city.lower():
                ...
                events.append({
                    ...
                    "start"   : _iso(start_dt),
                    "end"nano 
~/Documents/GitHub/svb-data/sources/lnsb_fetch.py
...
def lnsb_fetch(city_filter: str | None = None) -> list[dict]:
    print("• LiveNotesSB fetch")
    html = requests.get(SITE, headers={"User-Agent": "Mozilla/5.0"}, 
timeout=20).text
    soup = BeautifulSoup(html, "html.parser")

    # locate each date-section (<h4> heading followed by paragraphs)
    events: list[dict] = []
    for h4 in soup.find_all("h4"):
        hdr = h4.get_text(" ", strip=True).replace("\xa0", " ")
        if not HDR_RE.match(hdr):                       # skip unrelated 
h4s
            continue
